...
- Parameters are typed dataclasses — no silent dict key typos
"""
...
# Type alias — each growth rate entry is either a scalar or a (low, high) range
GrowthRate = Union[float, tuple[float, float]]


@dataclass
class DCFParameters:
    """
    Full parameter set for a single DCF scenario.

    growth_rates: per-year FCF growth rates, each either a scalar float
                  or a (low, high) uniform range, e.g. [(0.05, 0.10), 0.08, 0.06].
    terminal_growth: long-run growth rate, scalar or (low, high) range.
    """
    beta: float
    base_fcf: float
    growth_rates: list[GrowthRate]
    terminal_growth: GrowthRate
    ...
    def __post_init__(self) -> None:
        if not self.growth_rates:
            raise ValueError("growth_rates must contain at least one entry")
...
class MonteCarloUncertainty:
    """Standard-deviation overrides used when sampling in Monte Carlo."""
    fcf_growth_std: float = 0.03
    terminal_growth_std: float = 0.005
    beta_std: float = 0.1

    def __post_init__(self) -> None:
        for attr in ("fcf_growth_std", "terminal_growth_std", "beta_std"):
...
class ValuationResult:
    """Output of a single-scenario DCF calculation."""
...
class SimulationResult:
    """Output of a Monte Carlo run — per-share equity value distribution."""
    per_share_values: np.ndarray
    mean: float
    median: float
    std: float
    skew: float
...
class SensitivityResult:
    """
    Impact of ±delta shifts on mean per-share value.
    Maps label -> (downside_impact, upside_impact).
    """
...
class DCFEngine:
    ...
    def monte_carlo(
        ...
        base: DCFParameters,
        n: int = 10_000,
        unc: MonteCarloUncertainty | None = None,
    ...
    def sensitivity_analysis(self, base: DCFParameters, delta: float = 0.01) -> SensitivityResult:
        """
        ...
        # (attribute_name, original_value) pairs to perturb
        params_to_test: dict[str, str] = {
            'Beta':             'beta',
            'FCF Growth (Yr1)': 'growth_rates',
            'Terminal Growth':  'terminal_growth',
            'Cost of Debt':     'cost_of_debt',
        }

        impacts: dict[str, tuple[float, float]] = {}
        for label, attr in params_to_test.items():
            original = getattr(base, attr)
            down_params = self._with_perturbed(base, attr, self._perturb(original, -delta))
            up_params   = self._with_perturbed(base, attr, self._perturb(original, +delta))
            down_mean = self.monte_carlo(down_params, n=2_000, rng=fixed_rng()).mean
            up_mean   = self.monte_carlo(up_params,   n=2_000, rng=fixed_rng()).mean
            impacts[label] = (down_mean - base_mean, up_mean - base_mean)

        return SensitivityResult(impacts=impacts)
    ...
    def _project_fcf(self, base_fcf: float, growth_rates: list[GrowthRate]) -> np.ndarray:
    ...
    def _compute_terminal_value(self, last_fcf: float, terminal_growth: GrowthRate, wacc: float) -> float:
    ...
    @staticmethod
    def _perturb(value: GrowthRate | list[GrowthRate], delta: float) -> GrowthRate | list[GrowthRate]:
    ...
    @staticmethod
    def _with_perturbed(base: DCFParameters, attr: str, new_value) -> DCFParameters:
        """Return a shallow copy of base with one attribute replaced."""
        import dataclasses
        return dataclasses.replace(base, **{attr: new_value})

    @staticmethod
    def _build_results(per_share: np.ndarray) -> SimulationResult:
